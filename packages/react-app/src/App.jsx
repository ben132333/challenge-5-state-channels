  ...
  */
  ...
  function recieveVoucher(clientAddress) {
    ...
    function processVoucher(voucher) {
      ...
      if (signer !== clientAddress) {
        console.error("voucher signature is invalid");
        return;
      }
  ...
  function provideService(clientAddress) {
    ...
    if (channelInput) {
      ...
      // prevent continued service to clients who don't keep up with their payments
      // compare claimable to provided. If the client is paying too much, don't
      // provide the service.
      const maxReceived = 1000*document.getElementById(`claimable-${clientAddress}`).innerText;
      if ( wisdom.length - maxReceived > 99) {
        console.warn("client is not keeping up with payments. Stopped sending. Unpaid characters: ", wisdom.length - maxReceived);
        return;
      }

      // const existingVoucher = vouchers()[clientAddress];
      // if (existingVoucher !== undefined) {
      //   const updatedBalance = existingVoucher.updatedBalance;
      //   const wisdomLength = ethers.BigNumber.from(wisdom.length);
      //   const treshold = ethers.BigNumber.from(5);
      //   const unpaidWisdom = wisdomLength.sub(updatedBalance);

      //   if ( unpaidWisdom.gt(treshold) ) {
      //     console.warn("client is not keeping up with payments. Stopped sending.");
      //     return;
      //   }
      // }
